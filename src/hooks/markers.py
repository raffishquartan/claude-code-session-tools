...
``CCST_MARKERS_DIR`` env var, or ``$XDG_CACHE_HOME/claude/markers``) and carry
...
def markers_dir() -> Path:
    ...
    Resolution order: the ``CCST_MARKERS_DIR`` env var if set; otherwise
    ...
    absolute path.
    """
    custom = os.environ.get("CCST_MARKERS_DIR")
    if custom:
        return Path(custom)
