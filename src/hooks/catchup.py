...
def _log_failure(reason: str) -> None:
    # Route through ledger._hooks_dir() so CCST_HOOKS_DIR is honoured. telemetry.log_event
    # does NOT read CCST_HOOKS_DIR itself (only telemetry.main() does), so without an
    # explicit hooks_dir= this would write to the real ~/.cache/claude/logs/fires.jsonl even
    # under tests that set CCST_HOOKS_DIR - polluting the user's real ledger (§15).
    log_event(
        TelemetryEntry(
