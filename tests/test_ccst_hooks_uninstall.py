...
def test_remove_all_ccst_hooks_from_settings() -> None:
    settings = _settings_with_hooks("ccst hooks run session-end")
    removed: list = []
    new = _remove_hooks(settings, None, removed)
    assert removed == [("Stop", None, "ccst hooks run session-end")]
...
def test_remove_named_hook_only() -> None:
    ...
    removed: list = []
    new = _remove_hooks(settings, "session-end", removed)
    cmds = [r[2] for r in removed]
    assert "ccst hooks run session-end" in cmds
...
def test_hooks_uninstall_dry_run_default(tmp_path: Path) -> None:
    ...
    _write(tgt, _settings_with_hooks("ccst hooks run session-end"))
...
def test_hooks_uninstall_named_hook(tmp_path: Path) -> None:
    ...
    result = _run("hooks", "uninstall", "--hook", "session-end", "--target", str(tgt), "--apply")
    assert result.returncode == 0, result.stderr
    written = json.loads(tgt.read_text())
    remaining = [
        h["command"]
    ...
    assert "ccst hooks run session-end" not in remaining
