...
- **`.tag` file update (tag-changing operations only).** When the tag changes, the script writes `~/.claude/projects/<encoded-dst-cwd>/<uuid>.tag` with the new tag suffix. This lets `ccr <new-name>` resolve the jsonl by UUID immediately — before `/rename` has been run to update the jsonl's `custom-title`. Without this, `ccr <new-name>` would find the cc-sessions directory on disk but `claude --resume <new-name>` would fall back to the picker because the jsonl custom-title still has the old name. For RENAME-only, this overwrites any stale `.tag` file left from the old name; for MOVE+RENAME, it creates the file in the new project key directory.
